$(function(){
	...
	// remove sizing attributes from featured images
	$('.featured-img img').removeAttr('width');
	$('.featured-img img').removeAttr('height');	
	...
	var slideZone = $('.site-content').offset().top - $window.height() + 800;
	...
	$( ".featured-img img" ).each(function( index ) {
		var imgSrc = $(this).attr('src');
		$(this).parent().attr('data-src', imgSrc);
		
		$(this).parent().css('background-image', function(){
			var bg = ('url(' + $(this).data('src') + ')');
    	return bg;
		});
	});
